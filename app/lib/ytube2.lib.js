...
// https://www.googleapis.com/youtube/v3/playlistItems?part=snippet,status,contentDetails&maxResults=50&playlistId=PL674pQrgzNoTzcJyQoFd6BRRtdFwAWYwf&key=AIzaSyCW2pN3FIDIcCNiLjoHFLIzjbp7mRFLAMI
...
const playlistsearch = 'playlistItems?part=snippet,status,contentDetails'+maxrs;
const videoidsearch =`videos?part=snippet,contentDetails,status,statistics`;
...
async function loadFromVideoId(videoById,client_id, runnit ){
  ...
  const yURLx = youtubeURL + videoidsearch+`&id=${videoById}&key=${API_KEY}`;
  const dataBus=[];
  moredata = true;
  duration=0;
  //moredata = false;
...
module.exports  = {loadFromVideoId, loadFromPlaylistId, baseFromVideoId, ytDuration, baseFromPlaylistId,  loadApi, showData};
