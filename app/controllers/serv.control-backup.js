...
exports.xapiByChannel_id = (req, res) =>{
  
  const channel_id = req.params.channel_id;

  //console.log(" >>>>>>>>>>>>  000-1 <<<<<<<<<<<<<<<<<<< ",channel_id);
...
res.status(200);
 ...
 console.log(" we will register a channel ----- 01");
   ...
   console.log("u CHANNEL SAVER");
    ...
    const {channel_id,country ,owner,email,channel_name,password, station_start_time, station_duration_time, channel_info,contact_email,telephone_contact,address,city,start_time,timezone,logo,cbrand,thumbnail, adminrole2, adminrole3,adminemail2,adminemail3,ckeyword}  = req.body;
...
const feedup = {channel_id, country ,admin_role,owner,email,channel_name,password, station_start_time, station_duration_time, channel_info,contact_email,telephone_contact,address,city,start_time,timezone,logo,cbrand,thumbnail, admin_roles, ckeyword, ... feed_base} ;
...
cdone:(req, res) =>{
  console.log("moving to activateion done");
...
exports.channel = async (req,res)=>{
  ...
  if(req.user){
        user = req.user;
     const aa =  await station.getAdmin(user.username, (ans)=>{
...
exports.cloud = async (req, res) => {
 var  err="";
  ...
  if(req.user){
    ...
    user = req.user;
    const aa =  await station.getAdmin(user.username, (auth_admins)=>{
...
mxdata =  station.setChannel(data)
.then((maxData)=>{
...
get getStationDuration() {
  ...
  ,
  getAdmin:async (username,callback)=>{
  ...
  //let adminFor = await Serv.find({admin_roles:{$elemMatch:{username:username}}}).then((ans)=>{
    let adminFor = await Serv.find({'admin_roles.username' : username }).then((ans)=>{
  user_admin=[];
