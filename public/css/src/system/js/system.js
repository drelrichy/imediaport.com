//welcome
console.log ("************************************** system.js v 1.3 loaded ******************************");
let xchannels=[];
var rett;
...
function schannels(iptvc ,feedSelect) {
  ...
  //var options = $("#"+feedSelect).attr('options');
          ...
          xchannels[index] =  JSON.parse( dataArr[index]);
         xchan = xchannels[index];
         addOption(feedSelect, xchan.id, xchan.cid);
  ...
  case "ff":
    ...
    for (let index = 0; index < dataArr.length; index++) {
     ...
     xchan = xchannels[index];
    console.log(xchan.cid+"<--->"+feedSelect);
      ...
      if(xchan.cid==feedSelect){
        action.channel=xchan;
         ...
         //document.getElementsByName("cFname").value=xchan.cFname ;
          ...
          document.getElementById("cNumber").value= parseInt(xchan.cNumber) ;

          document.getElementById("key").value= parseInt(xchan.id) ;
          document.getElementById("cid").value = xchan.cid.trim(); 
          document.getElementById("xcid").value = xchan.cid.trim();

          document.getElementById("logoUrl").value = xchan.logoUrl.trim();
          formEvenCheck(document.getElementById("logoUrl"));
          document.getElementById("thumbUrl").value = xchan.thumbUrl.trim();
          document.getElementById("logoUrl").value = xchan.logoUrl.trim();
          document.getElementById("ckeyword").value = xchan.ckeyword.trim();
          let countrystate =xchan.country.trim();
          let locat= countrystate.split("|");
          let country=locat[0]; let stat = locat[1];
...
switch (action.modenum) {
  ...
  document.getElementById("cid").placeholder="channel ID?";
  //document.getElementById("cNumber").placeholder="";
  document.getElementById("cid").value="";
  document.getElementById("cNumber").value=1000+action.csize+1 ;
...
document.getElementById("messager").innerHTML= "<p style='color: red;'>you must select a channel to "+ action.mode +"</p>"
...
document.getElementById("messager").innerHTML = "<p style='color: blue;'>1Tv Network Manager,  "+ action.mode +" Channel |>> "+action.cid+" <<| ";
document.getElementById("cid").disabled=false;
document.getElementById("cNumber").disabled=false; 
document.getElementById("cidSelect").disabled=true;
//document.getElementById("cNumber").disabled=true;
...
case 1:
  ...
  //alert(action.cid);
 if(document.getElementById("cidSelect").length <3 ) schannels("sl","cidSelect");
...
document.getElementById("messager").innerText = "Welcome to 1Tv Network Manager, Delete Channel.";
...
let pagecall="";
     if(document.URL.indexOf("#")!=-1 ) { let pagecall = document.URL.substring(document.URL.indexOf("#")+1,document.URL.lengt);
return pagecall;
    }
}
...
let error= data.msg;
         ...
         document.getElementById("messager").innerHTML= "<p style='color: red;'>Sorry "+ action.mode +" for "+action.cid+" .. "+error+"</p>";
...
//============================ Build ajax Selector ============================================================
  ...
  console.log(feedback)
          ...
          function addOption(parentID, optionValue, optionLabel) {
          ...
          if(cid.length > 3){
             ...
             xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                   ...
                   if(response.status==1) {info.style.color="red"} else {info.style.color="green"};
...
//alert (upUrl);
 ...
 // document.getElementById("info").value= player.videoTitle;
  ...
  if(ct=0)  count();
    ...
    let rText = "";
      ...
      if (xurl.includes(playyt)&&xcode=="SHY") rText= xurl.replace(playyt, embedyt);
      if (xurl.includes(playlistyt)) rText= xurl.replace(playlistyt,playlistemebedyt);
 ...
 let embedObject = document.getElementById("iframe");
  ...
  case "SHI":
                    ...
                    if(getx("title",streamUrl)!="")  {eTitle=  getx("title",streamUrl);
  ...
  // case 5
  case "HI":         eSrc =   getx("src",streamUrl);
  
                  if(getx("title",embedObject.value)!="")  {eTitle=  getx("title",streamUrl);
                    if (eTitle!="")document.getElementById("info").value=eTitle;
                  };
