console.log("++==================== 1Network Manager - Main Modele version 1.0 ==================");
...
// setTimeout(  console.log('#cidSelect'),25000000000);
        ...
        xmodeSelect.onchange = function (event) { // modeaction();
...
// localizeaction ();
        ...
        xcidSelect.onchange = function (event) { // $('#cidSelect').val(mx2.trim()).change();
            console.log("Event ============");
...
// File input field trigger when the HTML element is clicked
    $("#dropBox").click(function () { // $("#fileToUpload").click();
    });
...
// Set up a handler for when the task for the request is complete
        xhr.onload = function () {
            if (xhr.status == 200) { // alert(file.name);
...
// alert(logoCase.style.backgroundImage);
                ...
                } else { // feedus.innerHTML = this.response.msg;
                }
            } else { //
            }
        };
...
function buildSelect(category, feedSelect) { // alert("show it > --"+phpserverlocation);
...
// url: phpserverlocation+"https://onetv.ng/ajaxgetformDB.php",
    $.ajax({
        ...
        dataType: "JSON",
        beforeSend: function () { //    alert();  //You can Add some functions to do before sending AJAX Request Eg: Page Loader
        },
...
// alert(feedback);
            ...
            } else { // alert(" error ooh "+feedback.status);
...
// console.log("|"+optionValue+"|");
    $("#" + parentID).append(`<option value="${optionValue}">
                        ${optionText}
                    </option>`);
}
...
function xembed(xurl, xcode) {
    ...
    let rText = "";
    if (xurl.includes(embedyt) && xcode == "SHYE") {
        rText = xurl;
    }
    if (xurl.includes(embedyt) && xcode == "SHYEP") {
        rText = xurl;
    }
...
// eSrc= eSrc.replace(playlistyt,playlistemebedyt);"SHYP":
    ...
    if (rText == "" && xurl.includes("youtu")) { // https://www.youtube.com/watch?v=
    ...
    console.log("EMBED REPORT =" + xurl + "<- in and out -> " + rText + " and Xcode =" + xcode);
...
// let eTitle="";
    ...
    let eSrc = "";
    let xxobj = streamUrl;
    if (xxobj.includes("https")) {
        xcode += "S";
    }
    if (xxobj.includes("http")) {
        xcode += "H";
    }
    if (xxobj.includes("iframe")) {
        xcode += "I";
    }
    if (xxobj.includes("youtube")) {
        xcode += "Y";
    }
    if (xxobj.includes("youtu.be")) {
        xcode += "Y";
    }
    if (xxobj.includes("embed")) {
        xcode += "E";
    }
    if (xxobj.includes(".m3u8")) {
        xcode += "8";
    }
    if (xxobj.includes("list=")) {
        xcode += "P";
    }
    if (streamUrl == "") {
        return "";
...
// 1
                ...
                if (eTitle != "") {
                    document.getElementById("cInfo").value = eTitle;
                }
...
// if (eSrc..includes('play'){ player.loadPlaylist(playlist, previousIndex+1)};
                ...
                break;
// case 4
            case "SHI": eSrc = getx("src", streamUrl);
                ...
                if (getx("title", streamUrl) != "") {
                    eTitle = getx("title", streamUrl);
                    if (eTitle != "") {
                        document.getElementById("info").value = eTitle;
                    }
                }
...
// case 7
            case "H8": day = "m3u8 HTTP Live Streaming media Streaming with http link ";
...
function submitChannel(ev) {
    ...
    var valid;
    var rout = "https://onetv.ng/server/ajaxsave2.php";
    let move = false;
    ...
    if (action.modenum == 0) {
        rout = "https://onetv.ng/server/ajaxsave2.php";
    } // get the route using attribute action
    if (action.modenum == 1) {
        rout = "https://onetv.ng/server/ajaxupdate2.php";
    } // ?key="+dbkey; //get the route using attribute action
    if (action.mode == 2) {
...
function checkCID(cidobj) {
    ...
    if (usernameRegex.test(cid)) {
        ...
        if (cid.length > 4) { // AJAX request



       var xhttp = new XMLHttpRequest();
          xhttp.open("GET", "https://onetv.ng/server/cid.ajaxdbcheck.php?channel_id=" + cid, true);
