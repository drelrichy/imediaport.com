if (typeof BASESERVER === "undefined")
{
    const BASESERVER  = location.origin;
}
else
{
    console.log(" and again  BASESERVER  =", BASESERVER );
}
...
programs_duration:''
 ...
 console.log("The call is =========>>",apiurl);
        ...
        this.assetList.forEach((program, p) => {
           // this.assetList[p].net_id = p;
            ...
            if (p == index) {
              ...
              if(isPlayer) player.loadVideoById(this.assetList[p].video_id, startTrim, this.assetList[p].stop);

             if(isCuePlayer) cueplayer.cueVideoById(this.assetList[p + 1].video_id, this.assetList[p + 1].start, this.assetList[p + 1].stop);
...
var isPlayer =(isPlayer) ? isPlayer : playtime.isPlayer ;
var isCuePlayer =(isCuePlayer)? isCuePlayer : playtime.isCuePlayer ;
 var isPrevPlayer=(isPrevPlayer)? isPrevPlayer: playtime.isPrevPlayer ;
...
sayuinput: async function( say) {
this.sayhold = document.getElementById('masterinput').value;
          //  this.universal_input = document.getElementById('masterinput');
    // console.log("we dey");
    // console.log( this.universal_input );
    $("#masterinput"). css("background-color","black");
    $("#masterinput"). css("color","white");

            return document.getElementById('masterinput').value = say;

        },
 restoreuinput: async function( say) {
          document.getElementById('masterinput').value =   this.sayhold ;

            $("#masterinput"). css("background-color","white");
            $("#masterinput"). css("color","black");
        },
