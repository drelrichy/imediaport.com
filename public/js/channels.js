...
console.log("************************************** channels.js V- 3.0 loaded ******************************");
var menutime = 0;
...
function xCB(iptvc, feedSelect) {

    let dataArr = channelbox;
...
// var xchannel;
    ...
    switch (iptvc) {
        ...
        case "all":
            ...
            for (let index = 0; index < dataArr.length; index = index + 1) {
                xchanbox[index] = JSON.parse(dataArr[index]);
                xchan = xchanbox[index];
                if (iptvc == "all") {
                    makeMenu("dynamicmenu", xchanbox[index], index);
                } // Add new values to selection box Two


            };
            $("#dynamicmenu").append('<a id="end" name="start" onclick="showmenu(this.name)"> back</a>');
        ...
        case "sortmenu": feedSelect = feedSelect.toLowerCase();
            ...
            for (let index = 0; index < dataArr.length; index = index + 1) {
                ...
                let searchit = xchan.cid + xchan.cFname + xchan.cInfo + xchan.ckeyword + xchan.ctype;
                if (feedSelect == "entertainment" && ifedey("movie,music,comedy", feedSelect)) {
                    isAGO = true;
                }
                searchit = searchit.toLowerCase();
                if (searchit.includes(feedSelect)) {
                    isAGO = true;
                }

                console.log(feedSelect + " from ->" + searchit);
                if (iptvc == "sortmenu" && isAGO) {
                    makeMenu("dynamicmenu", xchanbox[index], index);
                } // Add new values to selection box Two


            };
            $("#dynamicmenu").append('<a id="m-endmenu" onclick="showmenu("m-startmenu")"> back</a>');
...
// makeMenu("dynamicmenu", xchannels[index],index); // Add new values to selection box Two
        ...
        case "cid": feedSelect = feedSelect.trim();
...
function xhideMenu() {
    ...
    if (document.getElementById("mxcontrol").style.visibility != "hidden") {
        document.getElementById("mxcontrol").style.visibility = "hidden";
    }


    if (document.getElementById("navhost").style.visibility != "hidden") {
        document.getElementById("navhost").style.visibility = "hidden";
    }
    if (document.getElementById("ticker").style.visibility != "visible") {
...
// if(document.getElementById("mxcontrol").style.visibility!=null){
    ...
    if (document.getElementById("mxcontrol").style.visibility == "hidden") {
        document.getElementById("mxcontrol").style.visibility = "visible";
    }
// };
...
function startup() { // Get the reference to video
...
// make the video full screen
    ...
    } else { // Otherwise exit the full screen
...
function calledPage() {
    ...
    if (document.URL.indexOf("#") != -1 && callerPage == "") {
        currentChannel = document.URL.substring(document.URL.indexOf("#") + 1, document.URL.lengt);
...
// console.log (btns);
    for (var i = 0; i < btns.length; i = i + 1) { // var current = document.getElementsByClassName("active");
...
function setActive(url) { // console.log(url);
...
function getchannels(iptvc, feedSelect) { // <iframe width="726" height="408" src="https://www.youtube.com/embed/ZLGLG0FypqM?list=PLSXwhHiUvMLbEu6fuhZYVS4XW2V_iq9Ex&autoplay=1&volume=1&controls=1&title=0&share=0&open_playlist=0&random=0&autohide=1&enablejsapi=1" title="we are one Montage" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" style='position: absolute; top: 0; left: 0; width: 100%; height: 100%;'  allowfullscreen></iframe>
...
// alert(tvcode);
    ...
    if (tvcode.includes('?')) {
        codeadd = "&autoplay=1&volume=1&controls=0&disablekb=1&title=0&share=0&open_playlist=0&random=0&autohide=1&enablejsapi=1";
    } else {
        codeadd = "?autoplay=1&volume=1&controls=0&disablekb=1&title=0&share=0&open_playlist=0&random=0&autohide=1&enablejsapi=1";
    };
...
function xchanx(iptvc, feedSelect) {
    ...
    switch (iptvc) {
        ...
        case "all":
            ...
            for (let index = 0; index < dataArr.length; index = index + 1) {
                ...
                if (iptvc == "all") {
                    makeMenu("dynamicmenu", xchannels[index], index);
                } // Add new values to selection box Two
            ...
            if (feedback.status == 1) { // execute if status value 1 on Feed back data from database response
                ...
                switch (iptvc) {
                    ...
                    case "all":
                        ...
                        for (let index = 0; index < dataArr.length; index = index + 1) {
                            let xchan = JSON.parse(dataArr[index]);

                            if (iptvc == "all") {
                                makeMenu("dynamicmenu", xchan, index);
                            } // Add new values to selection box Two
...
// console.log(feedSelect+" from ->"+ searchit);
                    ...
                    case "sl":
                        ...
                        for (let index = 0; index < dataArr.length; index = index + 1) {
                            xchan = JSON.parse(dataArr[index]);
...
// <a tabindex="12" accesskey="2"   id="music"      class=""    onclick="  cPlay(this.id, myFunction)" href="#home"  >1Music-TV</a>
    ...
    $('#' + parentID).append('<a tabindex="' + nindex + '"       alt="' + channel.cid + '"  id="' + channel.cid + '"    tips="' + channel.cInfo + '"      name="' + channel.cFname + '"  class="dymen listmen"  onclick="  ciPlay(this,this.src)" href="#' + channel.cid + '"  >' + channel.cid.toUpperCase() + ' </a>');
...
function ifedey(array, feedSelect) {

    ari = array.split(",");
    logit("follow come=" + array);
    logit("ari =" + ari);
    logit("ari =" + feedSelect);

    let isAGO = false;

    for (const check of ari) {

        if (feedSelect.includes(check)) {
            isAGO = true;
        }
...
// <a tabindex="12" accesskey="2"   id="music"      class=""    onclick="  cPlay(this.id, myFunction)" href="#home"  >1Music-TV</a>
    let nindex = xindex + 10;
    $('#' + parentID).append('<a tabindex="' + nindex + '"       alt="' + channel.cid + '"  id="' + channel.cid + '"    tips="' + channel.cInfo + '"      name="' + channel.cFname + '"   onclick="  ciPlay(this,this.src)" href="#' + channel.cid + '"  >' + channel.cid.toUpperCase() + ' </a>');
...
function timerIncrement() { // idleTime = idleTime + 1;
    idleTime = idleTime + 1;
    if (idleTime > 10) {
