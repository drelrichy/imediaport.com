...
//============================ Build ajax Selector ============================================================
        ...
        function buildSelect(category ,feedSelect) {
         ...
         // alert("here --"+phpserverlocation);
            ...
            let brand = category; // $("#phoneBrand").val(); //get pre-set value from selection box One
              ...
              $.ajax({
                  type: "GET",
                  url: phpserverlocation+"ajaxgetformDB.php",
                  data: {
  ...
  //onclick(submitForm());
          ...
          var route 			  = "../server/ajaxsave.php"; //get the route using attribute action
...
console.log(error);                // You can put something here if there is an error from submitted request
  ...
  //alert(location.hostname);
    ...
    //name="fileToUpload" id="fileToUpload"
      ...
      // Open the connection
      xhr.open('POST', php_fileserver , true);
      ...
      xhr.onload = function () {
        if (xhr.status == 200) {
          ...
          upUrl =  hostpathname + this.responseText;
  //alert (upUrl);
